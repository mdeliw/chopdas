...
Another example: The `forEach` method takes a `Consumer` as an argument. The `Consumer` can be implemented as an expression or a method reference which is shorter and includes the name of the class containing the method.
...
//method reference
Stream.of(3, 1, 4, 1, 5, 9)
    .forEach(System.out::println); //Class::staticMethod
...
Stream.generate(Math::random)		// generate takes a static method as a Supplier
    .limit(10)
